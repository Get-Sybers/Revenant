...
# ---------------------------------------------------------------------------
# VMkatz.extract — subprocess mocking
# ---------------------------------------------------------------------------

class TestVMkatzExtract:
    """Tests that VMkatz.extract invokes the binary with the right arguments."""
    ...
    def test_disk_flag_passed(self, tmp_path):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(stdout="type,domain,username,nt_hash,lm_hash,sha1,plaintext\n", returncode=0)
...
# ---------------------------------------------------------------------------
# CLI integration (smoke test via argparse)
# ---------------------------------------------------------------------------
...
class TestCli:
    ...
    def test_extract_subcommand_runs(self, tmp_path, capsys):
        fake_bin = tmp_path / "vmkatz"
        fake_bin.touch()
        csv_out = _make_csv([
            {"type": "msv", "domain": "LAB", "username": "testuser",
    ...
    def test_extract_pwdump_format(self, tmp_path, capsys):
        fake_bin = tmp_path / "vmkatz"
        fake_bin.touch()
        csv_out = _make_csv([
            {"type": "msv", "domain": "LAB", "username": "admin",
